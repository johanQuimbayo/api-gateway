...
public class AuditController implements IAuditController {
    ...
    public Mono<PageResponseDTO<MessageDTO>> getMessagesByIdEntidad(@PathVariable String idEntidad, Pageable pageable, ServerHttpRequest originalRequest) {
        return auditClient.getMessagesByIdEntidad(idEntidad, pageable, originalRequest);
    }

    @GetMapping("/byRecurso/{recurso}")
    public Mono<PageResponseDTO<MessageDTO>> getMessagesByRecurso(@PathVariable String recurso, Pageable pageable, ServerHttpRequest originalRequest) {
